...
function serveStaticFile(string $path): bool
{
    ...
    // El frontend no tiene paso de build ni hash en los nombres: web/js/app.js
    // se llama igual antes y despues de cada cambio. Sin esto el navegador se
    // queda con la copia vieja y una correccion recien desplegada parece no
    // haber ocurrido. `no-cache` no prohibe guardar: obliga a revalidar, asi
    // que un archivo que no cambio se sigue resolviendo con un 304.
